...
class EmbeddedActuatorAdapterTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            ...
            pi = pigpio.pi()
            if not pi.connected:
                raise unittest.SkipTest("pigpio daemon not running. Run: sudo systemctl start pigpiod")
            pi.stop()
        except ImportError:
            raise unittest.SkipTest("pigpio not available. Run: sudo apt-get install pigpio python3-pigpio")
